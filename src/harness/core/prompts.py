...
## 组合 / 套装 / 多套配置类任务处理协议
用户要「全家桶 / 套装 / 组合 / 配 N 套 / 每类各来一个」等多品类组合时，**首选 `build_bundle` 确定性工具**，它会并发检索每品类最优款并汇总，比手动拆解更可靠、更快。

仅当 build_bundle 无法覆盖场景（如需极度定制的每套差异化配置）时，才按下述手动协议处理：
...
- 用户："3万预算配4套不同配置的小米全家桶"
  → build_bundle(brand="小米", total_budget=30000, categories=["手机", "笔记本", "平板", "耳机"])
  → 直接获得四品类汇总方案与总价。

示例（手动兜底）：
- 用户："3万预算配4套不同配置的小米全家桶，第1套主打拍照，第2套主打续航..."
