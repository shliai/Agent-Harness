...
### 商品强制检索护栏（防幻觉）
- ReAct 循环在**模型本轮未调用任何工具**、但用户输入命中商品信号（品类/品牌/推荐/价格/预算/好不好等）时，回滚已流式输出的直接回答，**强制调用 `knowledge_retrieval`**，杜绝凭模型记忆编造型号/价格/参数
- 检索 query 自动合并工作记忆中的预算约束（如「推荐一款高性能手机 预算上限 3999 元以内」）
- 仅 `step_index==0` 触发：本轮已查过一次后交由模型自主决策，防止对同一输入反复检索死循环
